...
def get_zip_info(file):
    file_list = file.namelist()  # list of files in zip
...
def start_stop(start, stop, size):
    # set tuple of empty values
    if start in NULL and stop in NULL:
        time = range(1, size)
    elif start not in NULL and stop in NULL:
        time = range(start, size)
    elif start in NULL and stop not in NULL:
        time = range(1, stop)
    return time
...
def process_zip(file_upload, output=None, mix='mix', method='Krief', plot=None, dirsize=None, start=None, stop=None):
    '''
    # File_upload
    # Output
    # Mix
    ...
    # dirsize
    '''
    # if a lot is not selected, stop
    if plot in NULL:
        return 0

    # amke plot string value lowercase
    plot = plot.lower()

    with zipfile.ZipFile(file_upload) as myzip:
        base, fname, ext, size = get_zip_info(myzip)

        # only for first file - because later files don't have porosoity or perm
        prefixin = f"{fname}1"
        prefixina = f"{fname}0"

        poro_file = f"{prefixina}.{ext}"
        with myzip.open(poro_file) as f_input:
            next(f_input)
        ...
        baseline_file = f"{prefixin}.{ext}"
        with myzip.open(baseline_file) as f_input:
            next(f_input)
    ...
    # change second value(13) to 'size' variable after testing #TODO
    time = range(1, 13)
    if output in NULL:
        foutput = f"./output_movie/{rtype}_{mix}_{method}_{plot}.mp4"
    else:
        if '.mp4' not in output:
            foutput = f"./output_movie/{output}.mp4"
        else:
            foutput = f"./output_movie/{output}"


##############################################################################
    with zipfile.ZipFile(file_upload) as myzip:
        def main(j):
            simulation_file = f"{fname}{j}.{ext}"
            print(j)

            # else:
            with myzip.open(simulation_file) as f_input:
                next(f_input)
                data = [line.strip().split(None, 14) for line in f_input]
    ...
    ##############################################################################
            if plot == 'vp' or plot == 'vs':
                for i in range(len(df1.x)):
                    ...
                    rho_water = 1+10**(-6)*(-80*Tem-3.3*Tem **
                                            2+0.00175*Tem**3+r1)
                    ...
                    wa = np.zeros((5, 4))
                    wa[:, 0] = [1402.85, 4.871, -0.04783,
                                1.487*10**-4, -2.197*10**-7]
        ...
        ##############################################################################
                    ...
                    v_clay = 1 - v_qua
                    # Shear modulus of quartz mineral (GPa)line 38
                    mu_qua = 44.0
    ...
    #############################################################################
            ...
            title = f"{rtype}_{mix}_{method}_{plot} Month {j}"
    ...
    ##############################################################################
        ...
        # time = start_stop(start, stop, size)
        time = range(1, 13)

        clip = DataVideoClip(time, main, fps=3)
        clip.write_videofile(foutput, fps=3)


# process_anim("./example/SA_Seismic.zip", mix='mix', method='Krief', plot='hydrogen')
